        ...
        <div
            ...
            <form wire:submit.prevent='store'>
                ...
                <div class="w-full max-w-xs sm:max-w-sm xl:max-w-xl form-control">
                    ...
                    <x-input wire:model.blur='event_sub_type_name' :error="$errors->get('event_sub_type_name')" />
                    <x-label-error :messages="$errors->get('event_sub_type_name')" />
                </div>
                <div class="w-full max-w-xs sm:max-w-sm xl:max-w-xl form-control">
                    <x-label-req :value="__('Event Subtype')" />
                    <x-select wire:model='event_type_id' :error="$errors->get('event_type_id')">
                        <option value="" selected>Select an option</option>
                        @foreach ($event_type as $eventtype)
                            <option value="{{ $eventtype->id }}">
                                {{ $eventtype->type_eventreport_name }}</option>
                        @endforeach
                    </x-select>
