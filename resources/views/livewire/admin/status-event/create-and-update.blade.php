    ...
    <div  wire:target="store" wire:loading.class="skeleton" class="p-2">
            ...
            <form wire:submit.prevent='store'>
                ...
                <div class="w-full max-w-xs sm:max-w-sm xl:max-w-xl form-control">
                    ...
                    <x-input wire:model.blur='status_name' :error="$errors->get('status_name')" />
                    <x-label-error :messages="$errors->get('status_name')" />
                </div>
                <div class="w-full max-w-xs sm:max-w-sm xl:max-w-xl form-control">
                    <x-label-req :value="__('Background Color')" />
                    <x-input wire:model.blur='bg_status' :error="$errors->get('bg_status')" />
                    <x-label-error :messages="$errors->get('bg_status')" />
                </div>
